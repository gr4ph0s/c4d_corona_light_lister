...
class LightManager(object):
    ...
    def get_corona_light(self):
        doc = c4d.documents.GetActiveDocument()
        corona_lights = self.search_light_in_hierarchy(doc.GetFirstObject(), const.LIGHT_ID_CORONA)
        buffer_corona_light = list()
        new_corona_light = list()

        for light in corona_lights:
            buffer = dict()
            buffer["light"] = light
            ...
            if old_id is False:
                buffer["id"] = 0
                new_corona_light.append(buffer)
            else:
                buffer["id"] = old_id
                buffer_corona_light.append(buffer)

        for i, light in enumerate(new_corona_light):
            light["id"] = len(buffer_corona_light) + i + 1

        buffer_corona_light += new_corona_light
        buffer_corona_light.sort(key=operator.itemgetter('id'))

        for i, light in enumerate(buffer_corona_light):
            light["id"] = i
            self.set_bc_tag(light["light"], i)

        return buffer_corona_light
