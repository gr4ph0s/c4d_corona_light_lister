...
class InteractionFunction(object):
    ...
    def color_temp_corona_interaction(self, dialog, doc, ui_id_color, ui_id_temp, clicked_id, c4d_lights):
        if ui_id_color <= clicked_id < ui_id_color + const.STEP:
            light_id = clicked_id - ui_id_color - 2
            ui_data = dialog.GetColorField(clicked_id)["color"]

            doc.AddUndo(c4d.UNDOTYPE_CHANGE, c4d_lights[light_id]["light"])
            c4d_lights[light_id]["light"][c4d.CORONA_LIGHT_B_COLOR] = True
            c4d_lights[light_id]["light"][c4d.CORONA_LIGHT_B_TEMPERATURE] = False
            c4d_lights[light_id]["light"][c4d.CORONA_LIGHT_COLOR] = ui_data
            c4d_lights[light_id]["light"].Message(c4d.MSG_UPDATE)

        elif ui_id_temp <= clicked_id < ui_id_temp + const.STEP:
            light_id = clicked_id - ui_id_temp - 2
            ui_data = dialog.GetLong(clicked_id)

            doc.AddUndo(c4d.UNDOTYPE_CHANGE, c4d_lights[light_id]["light"])
            c4d_lights[light_id]["light"][c4d.CORONA_LIGHT_B_TEMPERATURE] = True
            c4d_lights[light_id]["light"][c4d.CORONA_LIGHT_B_COLOR] = False
            c4d_lights[light_id]["light"][c4d.CORONA_LIGHT_TEMPERATURE] = ui_data
            c4d_lights[light_id]["light"].Message(c4d.MSG_UPDATE)
    ...
    def long_interaction_shape_corona(self, dialog, doc, ui_id, clicked_id, c4d_lights):
    ...
    def long_interaction_type_corona(self, dialog, doc, ui_id_type, clicked_id, c4d_lights):

        if ui_id_type <= clicked_id < ui_id_type + const.STEP:
            ui_id_shape = clicked_id + const.STEP
            light_id = clicked_id - ui_id_type - 2
            ui_data = dialog.GetLong(clicked_id)
            light_obj = c4d_lights[light_id]["light"]

            doc.AddUndo(c4d.UNDOTYPE_CHANGE, light_obj)
            light_obj[c4d.CORONA_LIGHT_TYPE] = ui_data

            if ui_data == 0:
                dialog.SetLong(ui_id_shape, light_obj[c4d.CORONA_LIGHT_AREA_SHAPE])

            elif ui_data == 1:
                dialog.SetLong(ui_id_shape, light_obj[c4d.CORONA_LIGHT_SECTOR_SHAPE])

            elif ui_data == 2:
                dialog.SetLong(ui_id_shape, light_obj[c4d.CORONA_LIGHT_OBJECT_SHAPE])