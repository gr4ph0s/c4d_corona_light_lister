...
with localimport('.') as importer:
    ...
    from src.Creation.CreationCorona import CreationCorona
    from src.Interaction.InteractionCorona import InteractionCorona
...
class mainDialog(c4d.gui.GeDialog):
    def __init__(self):
        ...
        self.creation = CreationCorona()
        self.interaction = InteractionCorona()
        self.corona_light = []
    ...
    def Message(self, msg, result):
        if msg.GetId() == c4d.BFM_GOTFOCUS:
            ...
            light_data = self.GetVisibleArea(const.GRP_TAB_CORONA_SCROLL_LIGHT)
            name_data = self.GetVisibleArea(const.GRP_TAB_CORONA_SCROLL_NAME)

            initial_name_y2 = name_data["y2"] - name_data["y1"]
            initial_light_y2 = light_data["y2"] - light_data["y1"]

            self.SetVisibleArea(const.GRP_TAB_CORONA_SCROLL_NAME,
                                name_data["x1"], new_value,
                                name_data["x2"], initial_name_y2 + new_value
                                )

            self.SetVisibleArea(const.GRP_TAB_CORONA_SCROLL_LIGHT,
                                light_data["x1"], new_value,
                                light_data["x2"], initial_light_y2 + new_value
                                )

        elif msg.GetId() == 1648576083:
            self.creation.refresh_corona_light(self, self.corona_light)
    ...
    def CreateLayout(self):
        ...
        self.corona_light = self.lm.get_corona_light()
        self.layers = self.lm.get_all_layers()


        self.SetTitle('Corona Light Lister v' + str(const.VERSION))
        self.SetTimer(500)

        self.create_menu()

        self.create_corona_data()
    ...
    def create_corona_data(self, refresh=False):
        if refresh:
        ...
        if self.GroupBegin(const.GRP_TAB_CORONA_GRP + 3, c4d.BFH_SCALEFIT | c4d.BFV_SCALEFIT, 300, 200, "Corona"):
            self.creation.create_corona_light(self, self.config.coronaConfig, self.corona_light, self.layers)
        self.GroupEnd()
    ...
    def Command(self, id, msg):
        if const.LIGHT_LISTER_CORONA_START < id < const.LIGHT_LISTER_CORONA_LIGHT_END:
            self.event_from_interaction = True

            self.doc.StartUndo()
            self.interaction.corona_interaction(self, self.doc, id, msg, self.corona_light, self.layers)
            self.doc.EndUndo()
            c4d.EventAdd()
    ...
    def refreshLightData(self):
        #store existing data
        save_scroll = [
            self.GetVisibleArea(const.GRP_TAB_CORONA_SCROLL_NAME),
            self.GetVisibleArea(const.GRP_TAB_CORONA_SCROLL_LIGHT)
        ]
        ...
        #redraw all light
        self.create_corona_data(True)

        #set scroll
        buffer_data = [
            const.GRP_TAB_CORONA_SCROLL_NAME,
            const.GRP_TAB_CORONA_SCROLL_LIGHT
        ]
...
class CoronaLightListerTag(c4d.plugins.TagData):
...
if __name__ == "__main__":
    ...
    c4d.plugins.RegisterCommandPlugin(  id=const.PLUGIN_ID,
                                        str="Corona Light Lister",
                                        help="List all corona lights in the scenes",
                                        info=0,
                                        dat=UILauncher_Main(), 
                                        icon=bmp)

    c4d.plugins.RegisterTagPlugin(id=const.PLUGIN_ID_TAG,
                                  str="TcoronaLightLister",
                                  g=CoronaLightListerTag,
                                  description="TcoronaLightLister",
                                  icon=None,
